...
<?php
	if(isset($_GET['id'])){
		$uid = $_GET['id'];
		$select = "select * from `employees` where `id`='$uid'";
	...
	$success="";
		if(isset($_POST['update'])){
			...
			$select = "select * from `employees` where `email`='$email'&& `phone`='$mobile' && `id`!='$uid'";
			$query = mysqli_query($con,$select);
			...
			if(empty($name)||empty($lname)||empty($company)||empty($email)||empty($mobile)){
				...
				if($count){
					...
					$update = "UPDATE `employees` SET `firstname`='$name',`lastname`='$lname',`company`='$company',`email`='$email',`phone`='$mobile' WHERE `id`='$uid'";
...
<head>
	...
	<title>Add name for Employees</title>
								...
								<div class="row">
									<div class="col-sm-6">
										<div class="form-group">
											...
											<input type="text" name="name" class="form-control" placeholder="Enter Name" value="<?php echo $data['firstname']?>">
										</div>
									</div>
									<div class="col-sm-6">
										<div class="form-group">
											<span class="form-label">Last Name</span>
											<input type="text" name="lname" class="form-control" placeholder="Enter Last Name" value="<?php echo $data['lastname']?>">
										</div>
									</div>
								</div>
								<div class="row">
									<div class="col-sm-6">
										<div class="form-group">
											...
											while($rescompnies=mysqli_fetch_array($querycompnies)){
												?>
												<option <?php if($data['company']==$rescompnies['id']){ echo "selected"; } ?> value="<?php echo $rescompnies['id']?>"><?php echo $rescompnies['name']?></option>
												<?php } ?>
											</select>
										</div>
									</div>
									<div class="col-sm-6">
										<div class="form-group">
											...
											<input type="text" name="email" class="form-control" placeholder="Enter E-Mail" value="<?php echo $data['email']?>">
										</div>
									</div>
									<div class="col-sm-6">
										<div class="form-group">
